...
export default function PersistentDrawerLeft() {
  ...
  const [open, setOpen, setFblock] = useStatusStore((state) => [
    state.open,
    state.setOpen,
    state.setFblock
  ])
  ...
  return (
    ...
    >
      ...
      {Object.keys(registry).map((text, index) => (
        ...
        >
          {registry[text].map((text2, index) => (
            <ListItem key={text + '_' + text2.address + '_' + text2.instId} disablePadding>
              <ListItemButton>
                <ListItemText
                  primary={`Addr: 0x${text2.address.toString(16)} inst: 0x${text2.instId.toString(
                    16
                  )}`}
                  onClick={() => {
                    setFblock({
