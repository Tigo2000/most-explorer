...
const SourceConnect: React.FC<Props> = ({ fBlock }) => {
  const [sourceNr, setSourceNr] = useState(1)

  const requestSourceConnect = (): void => {
    connectSource({sourceAddrHigh: fBlock?.targetAddressHigh, sourceAddrLow: fBlock.targetAddressLow, fBlockID: fBlock.fBlockID, instanceID: fBlock.instanceID, sourceNr})
  }
